...
- 简单说就是 3 个关键参数，对应 OpenAI `/v1/responses` 的入参：
...
一个感受是：传统产品依然是在定义和创造，然后交付给用户；
但是 AI 产品只是在帮助连接用户和模型，从一个创造者变成了传声筒。

还有一个想法是：没有垂直 Agent，只有通用 Agent。

Codex 其实只是给了 Agent 一个运行环境，然后提供各种 tools 和需要的上下文。从某种本质来看，和编程没有什么关系，最近火爆的 OpenClaw 算是某种证明？
